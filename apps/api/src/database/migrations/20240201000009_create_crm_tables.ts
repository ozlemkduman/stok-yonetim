...
export async function up(knex: Knex): Promise<void> {
  // CRM Contacts table
  if (!(await knex.schema.hasTable('crm_contacts'))) {
  ...
  if (!(await knex.schema.hasTable('crm_activities'))) {
  ...
  if (!(await knex.schema.hasTable('field_team_routes'))) {
  ...
  if (!(await knex.schema.hasTable('field_team_visits'))) {
  ...
  // Indexes
  await knex.schema.raw('CREATE INDEX IF NOT EXISTS idx_crm_contacts_customer ON crm_contacts(customer_id)');
  await knex.schema.raw('CREATE INDEX IF NOT EXISTS idx_crm_contacts_status ON crm_contacts(status)');
  await knex.schema.raw('CREATE INDEX IF NOT EXISTS idx_crm_activities_contact ON crm_activities(contact_id)');
  await knex.schema.raw('CREATE INDEX IF NOT EXISTS idx_field_routes_date ON field_team_routes(route_date)');
  await knex.schema.raw('CREATE INDEX IF NOT EXISTS idx_field_visits_route ON field_team_visits(route_id)');
}
