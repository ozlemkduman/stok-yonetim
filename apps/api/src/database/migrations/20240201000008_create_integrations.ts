...
export async function up(knex: Knex): Promise<void> {
  ...
  if (!(await knex.schema.hasTable('e_commerce_orders'))) {
  ...
  if (!(await knex.schema.hasTable('bank_statements'))) {
  ...
  // Indexes
  await knex.schema.raw('CREATE INDEX IF NOT EXISTS idx_integration_logs_integration ON integration_logs(integration_id)');
  await knex.schema.raw('CREATE INDEX IF NOT EXISTS idx_e_commerce_orders_integration ON e_commerce_orders(integration_id)');
  await knex.schema.raw('CREATE INDEX IF NOT EXISTS idx_e_commerce_orders_status ON e_commerce_orders(status)');
  await knex.schema.raw('CREATE INDEX IF NOT EXISTS idx_bank_statements_integration ON bank_statements(integration_id)');
  await knex.schema.raw('CREATE INDEX IF NOT EXISTS idx_bank_statements_match_status ON bank_statements(match_status)');
}
