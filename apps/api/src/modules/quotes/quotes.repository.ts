...
export class QuotesRepository {
  ...
  async findItemsByQuoteId(quoteId: string): Promise<QuoteItem[]> {
    return this.db.knex('quote_items').where('quote_id', quoteId);
  }

  async generateQuoteNumber(): Promise<string> {
    const today = new Date();
    const prefix = `TKL${today.getFullYear()}${String(today.getMonth() + 1).padStart(2, '0')}`;
    const [result] = await this.db.knex('quotes').whereILike('quote_number', `${prefix}%`).count('id as count');
    const count = parseInt(result.count as string, 10) + 1;
    return `${prefix}${String(count).padStart(4, '0')}`;
  }

  async create(data: Partial<Quote>, items: Partial<QuoteItem>[], trx?: Knex.Transaction): Promise<Quote> {
    const query = trx ? trx('quotes') : this.db.knex('quotes');
    const [quote] = await query.insert(data).returning('*');

    if (items.length > 0) {
      const itemsQuery = trx ? trx('quote_items') : this.db.knex('quote_items');
      await itemsQuery.insert(items.map((item) => ({ ...item, quote_id: quote.id })));
    }

    return quote;
  }

  async update(id: string, data: Partial<Quote>, trx?: Knex.Transaction): Promise<Quote> {
    const query = trx ? trx('quotes') : this.db.knex('quotes');
  ...
  async deleteItems(quoteId: string, trx?: Knex.Transaction): Promise<void> {
    const query = trx ? trx('quote_items') : this.db.knex('quote_items');
  ...
  async insertItems(items: Partial<QuoteItem>[], trx?: Knex.Transaction): Promise<void> {
    ...
    const query = trx ? trx('quote_items') : this.db.knex('quote_items');
    await query.insert(items);
  }

  async updateStatus(id: string, status: string, convertedSaleId?: string): Promise<void> {
    const updateData: any = { status, updated_at: this.db.knex.fn.now() };
    if (convertedSaleId) {
      updateData.converted_sale_id = convertedSaleId;
    }
    await this.db.knex('quotes').where('id', id).update(updateData);
  }

  async delete(id: string): Promise<void> {
    await this.db.knex('quotes').where('id', id).delete();
  }
