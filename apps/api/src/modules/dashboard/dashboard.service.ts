...
export class DashboardService {
  ...
  async getSummary() {
    ...
    const [
      ...
      this.db.knex('customers').where('is_active', true).count('id as count').first(),

      this.db.knex('products').where('is_active', true).count('id as count').first(),

      this.db.knex('products')
        .where('is_active', true)
        .whereRaw('stock_quantity <= min_stock_level')
        .count('id as count')
        .first(),

      this.db.knex('customers')
        .where('balance', '<', 0)
