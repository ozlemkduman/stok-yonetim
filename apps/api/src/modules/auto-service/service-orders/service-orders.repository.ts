...
export class ServiceOrdersRepository extends BaseTenantRepository<ServiceOrder> {
  ...
  async findByIdDetailed(id: string): Promise<ServiceOrder | null> {
    const result = await this.baseSelect().where('service_orders.id', id).first();
    return result || null;
  }
  ...
  async createOrder(data: Partial<ServiceOrder>): Promise<ServiceOrder> {
    ...
    const [order] = await this.knex('service_orders').insert(insertData).returning('*');
    return order;
  }

  async updateOrder(id: string, data: Partial<ServiceOrder>): Promise<ServiceOrder | null> {
