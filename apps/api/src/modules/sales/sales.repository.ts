...
export class SalesRepository {
  ...
  async findAll(params: { page: number; limit: number; search?: string; customerId?: string; status?: string; startDate?: string; endDate?: string; sortBy: string; sortOrder: 'asc' | 'desc' }): Promise<{ items: Sale[]; total: number }> {
  ...
  async findById(id: string): Promise<Sale | null> {
    return this.db.knex('sales').leftJoin('customers', 'sales.customer_id', 'customers.id').select('sales.*', 'customers.name as customer_name').where('sales.id', id).first() || null;
  }

  async findItemsBySaleId(saleId: string): Promise<SaleItem[]> {
    return this.db.knex('sale_items').leftJoin('products', 'sale_items.product_id', 'products.id').select('sale_items.*', 'products.name as product_name').where('sale_items.sale_id', saleId);
  }
