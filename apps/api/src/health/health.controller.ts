...
export class HealthController {
  ...
  async seed() {
    ...
    const existingCustomers = await knex('customers').count('* as count').first();
    if (existingCustomers && Number(existingCustomers.count) > 0) {
  ...
  private async runFullSeed() {
    ...
    await knex('payments').insert([
      ...
      { customer_id: customers[2].id, sale_id: sales[2].id, payment_date: '2024-01-17', amount: 7800, method: 'bank_transfer', notes: 'Havale' },
      { customer_id: customers[4].id, sale_id: sales[4].id, payment_date: '2024-01-20', amount: 25200, method: 'cash', notes: 'Pesin odeme' },
    ]);

    // 9. RETURNS
    const returns = await knex('returns').insert([
    ...
    await knex('expenses').insert([
      ...
      { category: 'fatura', description: 'Elektrik faturasi', amount: 2500.0, expense_date: '2024-01-15', is_recurring: false },
      { category: 'maas', description: 'Calisan maasi', amount: 25000.0, expense_date: '2024-01-30', is_recurring: true, recurrence_period: 'aylik' },
      { category: 'vergi', description: 'KDV odemesi', amount: 8500.0, expense_date: '2024-01-25', is_recurring: false },
    ...
    await knex('account_movements').insert([
      { account_id: accounts[0].id, movement_type: 'gelir', category: 'satis', amount: 18000, balance_after: 28000, description: 'Satis - INV202401001', movement_date: '2024-01-15' },
      { account_id: accounts[0].id, movement_type: 'gelir', category: 'satis', amount: 25200, balance_after: 53200, description: 'Satis - INV202401005', movement_date: '2024-01-20' },
      { account_id: accounts[1].id, movement_type: 'gelir', category: 'satis', amount: 62700, balance_after: 162700, description: 'Satis - INV202401002', movement_date: '2024-01-16' },
    ]);
    ...
    await knex('crm_activities').insert([
      { contact_id: contacts[0].id, type: 'call', subject: 'Ilk gorusme', description: 'Urun tanitimi yapildi', status: 'completed', scheduled_at: '2024-01-15 10:00:00', completed_at: '2024-01-15 10:30:00', duration_minutes: 30 },
      { contact_id: contacts[0].id, type: 'email', subject: 'Teklif gonderimi', description: 'Fiyat teklifi gonderildi', status: 'completed', scheduled_at: '2024-01-16 14:00:00', completed_at: '2024-01-16 14:15:00' },
      { contact_id: contacts[1].id, type: 'meeting', subject: 'Yuz yuze toplanti', description: 'Urun demosu yapilacak', status: 'planned', scheduled_at: '2024-02-01 14:00:00', duration_minutes: 60 },
      { contact_id: contacts[2].id, type: 'note', subject: 'Musteri oldu', description: 'Satis tamamlandi', status: 'completed', completed_at: '2024-01-18 16:00:00' },
    ]);
    ...
    await knex('stock_movements').insert([
      { product_id: products[0].id, warehouse_id: warehouses[0].id, movement_type: 'sale', quantity: -1, stock_after: 14, reference_type: 'sale', reference_id: sales[0].id, notes: 'Satis' },
      { product_id: products[1].id, warehouse_id: warehouses[0].id, movement_type: 'sale', quantity: -1, stock_after: 5, reference_type: 'sale', reference_id: sales[1].id, notes: 'Satis' },
      { product_id: products[0].id, warehouse_id: warehouses[0].id, movement_type: 'return', quantity: 1, stock_after: 15, reference_type: 'return', reference_id: returns[0].id, notes: 'Iade' },
    ]);
